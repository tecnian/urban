	...
	$.fn.mpcLayerSlider = function(options) {
		...
		function init() {
			...
			prepareVideos();
                        ...
                        if (settings.caption)
                        {                            
                            $('#mpc_slide_caption_0').css('display','block');                            
                        }
		...
		function addBehavior() {
                        ...
                        $prev_slide.on('click', function() {
                            ...
                            $('.mpc_slide').attr('data-transition','slideLeft');
			})
		...
		function changeSlide(new_index) {
			...
			$new_slide = $slides.eq(index);
                        ...
                        initTransition($current_slide, $new_slide, index);
		...
		function initTransition($current, $next, current_index) {
			var transition = $next.data('transition'),
				 easing = $next.data('easing');
				 time = settings.transitionTime;

                        transition = $('#mpc_slide_item_' + current_index).attr('data-transition');
			...
			if(settings.revertAnimation)
				reanimateSlide($current);
                        ...
                        if (settings.caption)
                        {
                            $('.mpc_slide_caption').css('display','none');
                            $('#mpc_slide_caption_' + current_index).css('display','block');                            
                        }
